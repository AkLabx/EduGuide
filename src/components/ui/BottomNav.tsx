...
import { Home, BookOpen, HelpCircle, User, PenTool } from 'lucide-react';
...
const navItems = [
  ...
  { id: 'hw', label: 'HW', icon: PenTool, path: '/homework' },
...
export function BottomNav() {
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50 border-t border-slate-200/50 bg-white/95 pb-safe pt-2 backdrop-blur-md dark:border-slate-800/50 dark:bg-slate-950/95 shadow-[0_-2px_10px_rgba(0,0,0,0.05)]">
      <nav className="mx-auto flex max-w-md items-center justify-around px-1 pb-2">
        {navItems.map((item) => {
          ...
          return (
            <NavLink
              ...
              className={({ isActive }) =>
                cn(
                  "relative flex flex-col items-center justify-center space-y-1 w-[20%] p-1 transition-colors duration-300",
                  isActive 
            ...
            >
              {({ isActive }) => (
                <>
                  ...
                  <span className="text-[10px] leading-tight truncate px-1 w-full text-center">{item.label}</span>
                </>
              )}
            </NavLink>
          );
